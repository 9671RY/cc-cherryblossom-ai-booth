export async function onRequestPost(context) {
  try {
    ...
    const prompt = "Please convert this coloring book image into a soft, warm watercolor painting style. Completely REMOVE all black lines and outlines, keeping only the watercolor strokes. (aspect ratio is 1:1)";
    ...
    const filename = `result3-${uploadId}-${Date.now()}.${fileExtension}`;
    ...
    const result3Url = `/cdn-cgi/image/width=800/cherryblossom-photos/${filename}`;

    // 3. DB 업데이트
    if (env.D1_DB && uploadId) {
      await env.D1_DB.prepare(
        "UPDATE photos SET result3_url = ? WHERE id = ?"
      ).bind(result3Url, uploadId).run();
    }

    return new Response(JSON.stringify({ 
      uploadId,
      result3Url,
      result3Base64: generatedBase64
    }), {
      headers: { 'Content-Type': 'application/json' }
    });

  } catch (error) {
    console.error("Step 3 API Error:", error.stack || error);
